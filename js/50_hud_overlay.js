...
function drawMenu(){
  overlay();txt('大 航 海 时 代',960,280,110,'#ffffff','#5f3a17',14);txt('V6.5 · 敌船战斗 AI',960,365,38,'#ffd23e','#5f3a17',6);
  const lines=['🏴‍☠️ 突击艇会更快扫描并抢占较窄的可用接舷空隙',
    '⚓ 巨舰需要更宽净空，找不到大位置时会在外侧等待',
    '🧭 后方接舷船会避开前船正在占用的目标，不再硬顶同一路线',
    '💥 多艘炮舰会自动错开射击航位，并保持远程炮击距离',
    '🪝 V6.3 智能接舷 + V6.4 登船队列继续保留'];
  lines.forEach((l,i)=>txt(l,960,445+i*48,25,'#e8f4fb','#0e3a52',5));bigButton(BTN_START,'开始连续海战！');txt('类型化敌船 AI · 智能接舷 · 登船队列',960,835,20,'#9cc4d8',null,0);
}
