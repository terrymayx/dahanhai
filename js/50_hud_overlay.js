...
function drawMenu(){
  overlay();txt('大 航 海 时 代',960,280,110,'#ffffff','#5f3a17',14);txt('V6.6 · 甲板近战 AI',960,365,38,'#ffd23e','#5f3a17',6);
  const lines=['⚔️ 海盗进入甲板后会分散围攻，不再全部叠在同一个点',
    '🏹 弓箭手会主动保持远距离，海盗逼近时自动后撤',
    '💣 炮手会保持爆弹射程，并优先选择海盗较密集的位置',
    '🛡️ 船长主动迎击近身目标；船员之间会保持站位间隔',
    '🪝 V6.3 智能接舷 + V6.4 登船队列 + V6.5 敌船 AI 全部保留'];
  lines.forEach((l,i)=>txt(l,960,445+i*48,25,'#e8f4fb','#0e3a52',5));bigButton(BTN_START,'开始连续海战！');txt('甲板近战 AI · 类型化敌船 AI · 智能接舷',960,835,20,'#9cc4d8',null,0);
}
