(function(root){
  ...
  function isOuter(ship,cell){
    if(!ship||!cell||!ship.cellMap)return false;
    for(const [dx,dy] of DIRS)if(!ship.cellMap[key(cell.gx+dx,cell.gy+dy)])return true;
    return false;
  }
  ...
  function rebuildCandidates(ship){
    ...
    ship.__v100ImmersionCandidates=list;
    ship.__v100ImmersionTopologyRevision=Number(ship.__v99TopologyRevision)||0;
    return list;
  }
  ...
  function capsizeStage(ship){
    ...
    if(a>=CAPSIZE_LOCK-.001)return 'locked';
    if(a>=CAPSIZE_DANGER)return 'capsizing';
    if(a>=IMMERSION_START)return 'danger';
    if(a>=LIST_START)return 'listing';
    return 'stable';
  }

  function handlingMultiplier(ship){
    const stage=capsizeStage(ship);
    if(stage==='locked')return .12;
    if(stage==='capsizing')return .28;
    if(stage==='danger')return .55;
    if(stage==='listing')return .78;
    return 1;
  }
  ...
  function addImmersionFx(state,ship,item){
    ...
    state.fx.push({k:'foam',x:p.x,y:p.y,t:0,dur:.55,r:7});
    state.fx.push({k:'waterRing',x:p.x,y:p.y,t:0,dur:.62,r:12});
  ...
  function updateImmersion(state,ship,dt){
    ...
    for(const comp of comps){comp.submergedOpenings=0;comp.__v100ImmersionSide=0;}

    let roll=Number(ship.__v99Roll)||0;
    const absRoll=Math.abs(roll),sideSign=Math.sign(roll)||1,half=transverseHalf(ship);
    ...
    if(absRoll>=IMMERSION_START*.82){
      for(const item of ship.__v100ImmersionCandidates){
        ...
        const side=clamp(item.transverse/half,-1,1);
        if(side*sideSign<.22)continue;
        const weakness=damageReadiness(cell);
        if(weakness<.18)continue;
        const sideDepth=Math.max(0,side*sideSign-.18);
        const angleDepth=Math.max(0,(absRoll-IMMERSION_START*.82)/Math.max(.01,MAX_PHYSICAL_ROLL-IMMERSION_START*.82));
        const sinkDepth=clamp((Number(ship.__v99SinkOffset)||0)/18,0,.65);
        const immersion=sideDepth*.62+angleDepth*.72+sinkDepth*.35;
        const threshold=.56+(1-weakness)*.34;
        if(immersion<threshold)continue;
        const C=root.V99Compartments||null;
        const idx=C&&typeof C.compartmentIndex==='function'?C.compartmentIndex(ship,cell):0;
        const comp=comps[idx];if(!comp)continue;
        comp.submergedOpenings++;
        comp.__v100ImmersionSide=sideSign;
        count++;if(!first)first=item;
      }
    }
    ship.__v100SubmergedOpeningCount=count;

    const sideDanger=(comps||[]).reduce((m,c)=>Math.max(m,Number(c.water)||0),0);
    const flooded=clamp(Number(ship.floodLevel)||0,0,1);
    let physical=Number(ship.__v100PhysicalRoll)||roll;
    const response=1-Math.exp(-dt*4.2);
    physical+=(roll-physical)*response;
    if(Math.abs(physical)>=CAPSIZE_DANGER&&sideDanger>.48){
      const sign=Math.sign(physical)||sideSign;
      const runaway=(.045+Math.max(0,sideDanger-.48)*.17+count*.004+flooded*.035)*dt;
      physical+=sign*runaway;
    }
    if(count>=2&&absRoll>=IMMERSION_START){
      physical+=(Math.sign(physical)||sideSign)*Math.min(.035,count*.0038)*dt;
    }
    ...
    ship.__v100PhysicalRoll=physical;
    ship.__v99Roll=physical;
    ship.__v97FloodRoll=physical;
    ship.__v100CapsizeStage=capsizeStage(ship);
    ship.__v100HandlingMultiplier=handlingMultiplier(ship);
    ...
    ship.__v100ImmersionFxT-=dt;
    if(count>0&&first&&ship.__v100ImmersionFxT<=0){
      addImmersionFx(state,ship,first);
      ship.__v100ImmersionFxT=.24;
    }

    if(ship.__v100CapsizeTimer>=LOCK_DELAY){
      const Buoy=root.V99Buoyancy||null;
      if(Buoy&&typeof Buoy.forcePhysicalSink==='function')Buoy.forcePhysicalSink(state,ship,'capsize');
    }
    return count;
  }

  if(B&&typeof B.update==='function'&&!B.__v100SinkingWrapped){
    B.__v100SinkingWrapped=true;
    const originalUpdate=B.update;
    B.update=function(state,dt){
      originalUpdate(state,dt);
      if(!state||!(dt>0))return;
      updateImmersion(state,state.player,dt);
      for(const ship of state.enemies||[])updateImmersion(state,ship,dt);
    };
  }
