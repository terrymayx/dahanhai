(function(root){
  ...
  function estimateFlightTime(state,p){
    const speed=Math.hypot(p.vx,p.vy)||1;let target=null,tx=null,ty=null;
    if(p.side==='player'){
      const enemies=(state.enemies||[]).filter(s=>s.state==='active');if(state.aim)target=enemies.find(s=>s.id===state.aim.shipId)||null;if(!target)target=enemies[0]||null;
      if(target&&state.aim&&state.aim.shipId===target.id&&Number.isFinite(state.aim.lx)&&Number.isFinite(state.aim.ly)){const a=Grid.localToWorld(target,state.aim.lx,state.aim.ly);tx=a.x;ty=a.y;}
    }else target=state.player&&state.player.state==='active'?state.player:null;
    if(target){if(tx==null){tx=target.x;ty=target.y;}return Math.max(.28,Math.min(2.2,Math.hypot(tx-p.x,ty-p.y)/speed));}
  ...
  function targetsFor(state,p){if(p.side==='player')return (state.enemies||[]).filter(s=>s.state==='active');return state.player&&state.player.state==='active'?[state.player]:[];}
  function updateArc(p,dt){p.prevZ=p.z||0;if(!(p.arcHeight>0&&p.flightTime>0)){p.z=0;p.vz=0;return;}p.arcAge=Math.min(p.flightTime,(p.arcAge||0)+dt);const t=p.arcAge;p.z=Math.max(0,p.initialVz*t-.5*p.gravity*t*t);p.vz=p.initialVz-p.gravity*t;}
  function updateTrail(p,dt){if(!p.trail)p.trail=[];for(const t of p.trail)t.t=(t.t||0)+dt;p.trail=p.trail.filter(t=>t.t<t.dur);p.trailT=(p.trailT||0)-dt;if(p.trailT<=0){p.trail.push({x:p.x,y:p.y,z:p.z||0,t:0,dur:.28});if(p.trail.length>8)p.trail.splice(0,p.trail.length-8);p.trailT+=.05;}}

  function firstPhysicalHit(ship,x0,y0,x1,y1){
    if(!ship||ship.state==='gone')return null;
    const a=Grid.worldToLocal(ship,x0,y0),b=Grid.worldToLocal(ship,x1,y1),dx=b.x-a.x,dy=b.y-a.y,len=Math.hypot(dx,dy),step=Math.max(1.25,(ship.cellSize||8)*.22),samples=Math.max(1,Math.ceil(len/step));let lastKey='';
    for(let i=0;i<=samples;i++){const t=i/samples,lx=a.x+dx*t,ly=a.y+dy*t,g=Grid.localToGrid(ship,lx,ly),k=g.gx+','+g.gy;if(k===lastKey)continue;lastKey=k;const cell=ship.cellMap&&ship.cellMap[k];if(cell&&cell.alive&&!cell.detachedGone)return{ship,cell,t,worldX:x0+(x1-x0)*t,worldY:y0+(y1-y0)*t};}
    return null;
  }

  function reflectProjectile(p,normal){
    normal=normal||{x:0,y:0};let nx=Number(normal.x)||0,ny=Number(normal.y)||0;const nd=Math.hypot(nx,ny)||1;nx/=nd;ny/=nd;const dot=p.vx*nx+p.vy*ny;let rvx=p.vx-2*dot*nx,rvy=p.vy-2*dot*ny;
    if(!Number.isFinite(rvx)||!Number.isFinite(rvy)||(!rvx&&!rvy)){rvx=-p.vx;rvy=-p.vy;}p.vx=rvx*.55;p.vy=rvy*.55;p.damage=Math.max(.1,(Number(p.damage)||0)*.45);p.attackPower=Math.max(.1,(Number(p.attackPower)||Number(p.damage)||0)*.45);p.__v99Ricocheted=true;p.life=Math.min(p.life,.9);const speed=Math.hypot(p.vx,p.vy)||1;p.x+=p.vx/speed*2.2;p.y+=p.vy/speed*2.2;
  }

  function hitStructuralChunk(state,p,x0,y0){
    const ChunkDamage=root.V101ChunkDamage||null;if(!ChunkDamage||typeof ChunkDamage.hitTestSegment!=='function')return false;
  ...
  function updateAll(state,dt){
    ...
    for(const p of state.projectiles){
      if(p.dead)continue;const x0=p.x,y0=p.y;updateArc(p,dt);p.x+=p.vx*dt;p.y+=p.vy*dt;p.life-=dt;updateTrail(p,dt);
      let bestHit=null;
      for(const ship of targetsFor(state,p)){const hit=firstPhysicalHit(ship,x0,y0,p.x,p.y);if(!hit)continue;const hk=(ship.id||ship.kind)+':'+hit.cell.gx+','+hit.cell.gy;if(p.hitCells[hk])continue;if(!bestHit||hit.t<bestHit.t)bestHit=hit;}
      ...
      if(!p.dead&&!p.didHit&&!p.splashDone&&p.arcHeight>0&&p.arcAge>=p.flightTime){p.splashDone=true;if(typeof state.onProjectileSplash==='function')state.onProjectileSplash(p,{x:p.x,y:p.y});p.dead=true;}
