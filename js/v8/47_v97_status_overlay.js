(function(root){
  ...
  function drawStatus(state){
    ...
    const attack=A&&typeof A.getAttack==='function'?A.getAttack(state):Math.round(state.playerShellAttack||72),auto=A&&typeof A.isAuto==='function'?A.isAuto(state):!!state.playerAttackAuto,radiusScale=E&&typeof E.blastRadiusScale==='function'?E.blastRadiusScale({side:'player',attackPower:attack,damage:attack}):1;
    const pf=Math.round(Math.max(0,Math.min(1,state.player&&state.player.floodLevel||0))*100),leaks=(state.player&&state.player.__v97LeakCount)||0;ctx.font='700 17px "Microsoft YaHei",sans-serif';ctx.textBaseline='middle';ctx.textAlign='left';ctx.fillStyle='#ffd65a';ctx.fillText(`炮攻 ${attack} ${auto?'AUTO':'手动'} · 爆幅 ${radiusScale.toFixed(1)}×`,250,96);
  ...
  root.V97StatusOverlay={drawStatus,targetPreviewCell,activeTarget,targetArmorPreview,previewProjectile,compartmentSummary,structuralSummary,physicalSummary,breakStageLabel};
