(function(root){
  ...
  function drawHudOverlay(ctx,state){
    ctx.fillStyle='rgba(5,30,48,.90)';ctx.beginPath();ctx.roundRect(26,24,650,112,18);ctx.fill();ctx.font='700 27px "Microsoft YaHei",sans-serif';ctx.textAlign='left';ctx.textBaseline='middle';ctx.fillStyle='#fff';ctx.fillText('V9.4 · 高耐久细碎船体',50,55);ctx.font='700 21px "Microsoft YaHei",sans-serif';ctx.fillStyle='#dff7ff';ctx.fillText(`我方结构 ${Math.round(G.integrity(state.player)*100)}%`,50,96);ctx.textAlign='right';ctx.fillStyle='#ffd65a';ctx.fillText(`击沉 ${state.kills}   金币 ${state.gold}`,650,96);
    ctx.fillStyle='rgba(7,37,53,.78)';ctx.beginPath();ctx.roundRect(700,18,1000,52,14);ctx.fill();ctx.font='700 21px "Microsoft YaHei",sans-serif';ctx.textAlign='center';ctx.fillStyle='#f3fbff';ctx.fillText('细化格网 · 甲板高耐久 · 炮击可点燃 · 持续燃烧后才会烧穿',1200,44);
    ctx.fillStyle='rgba(7,37,53,.76)';ctx.beginPath();ctx.roundRect(310,C.H-68,1300,42,12);ctx.fill();ctx.font='700 19px "Microsoft YaHei",sans-serif';ctx.fillStyle='#e9f8ff';ctx.fillText('格子更小更密，破坏更细碎；甲板不会像大方块一样一格一格突兀消失',960,C.H-47);
  }
