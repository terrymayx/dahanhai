(function(root){
  ...
  function formatAimInfo(ship,cell){
    ...
    const fire=cell.burning?' · 燃烧':'';
    const primary=`${type} ${hp} / ${max} · ${STAGE_LABEL[stage]||'完整'}${fire}`;let detail='';
  ...
  function drawAimInfo(ctx,state){const resolved=resolveAimCell(state);if(!resolved)return;const {ship,cell}=resolved,info=formatAimInfo(ship,cell),world=G.cellCenterWorld(ship,cell),x=Math.max(170,Math.min(C.W-170,world.x)),y=Math.max(120,world.y-58),width=info.detail?300:290,height=info.detail?58:38;ctx.fillStyle='rgba(7,30,43,.88)';ctx.beginPath();ctx.roundRect(x-width/2,y-height/2,width,height,10);ctx.fill();ctx.strokeStyle=cell.burning?'rgba(255,126,46,.95)':info.stage==='critical'?'rgba(255,185,84,.95)':'rgba(222,245,255,.65)';ctx.lineWidth=1.5;ctx.stroke();ctx.textAlign='center';ctx.textBaseline='middle';ctx.font='700 17px "Microsoft YaHei",sans-serif';ctx.fillStyle='#f5fbff';ctx.fillText(info.primary,x,y-(info.detail?10:0));if(info.detail){ctx.font='700 14px "Microsoft YaHei",sans-serif';ctx.fillStyle=info.detail==='危险'?'#ffd06a':'#bfe9ff';ctx.fillText(info.detail,x,y+14);}}
  function drawHudOverlay(ctx,state){
    ctx.fillStyle='rgba(5,30,48,.90)';ctx.beginPath();ctx.roundRect(26,24,650,112,18);ctx.fill();ctx.font='700 27px "Microsoft YaHei",sans-serif';ctx.textAlign='left';ctx.textBaseline='middle';ctx.fillStyle='#fff';ctx.fillText('V9.3 · 甲板燃烧耐久',50,55);ctx.font='700 21px "Microsoft YaHei",sans-serif';ctx.fillStyle='#dff7ff';ctx.fillText(`我方结构 ${Math.round(G.integrity(state.player)*100)}%`,50,96);ctx.textAlign='right';ctx.fillStyle='#ffd65a';ctx.fillText(`击沉 ${state.kills}   金币 ${state.gold}`,650,96);
    ctx.fillStyle='rgba(7,37,53,.78)';ctx.beginPath();ctx.roundRect(700,18,1000,52,14);ctx.fill();ctx.font='700 21px "Microsoft YaHei",sans-serif';ctx.textAlign='center';ctx.fillStyle='#f3fbff';ctx.fillText('甲板 3×耐久 → 炮击掉血 → 概率起火 → 持续燃烧 → 0HP破洞进水',1200,44);
    ctx.fillStyle='rgba(7,37,53,.76)';ctx.beginPath();ctx.roundRect(310,C.H-68,1300,42,12);ctx.fill();ctx.font='700 19px "Microsoft YaHei",sans-serif';ctx.fillStyle='#e9f8ff';ctx.fillText('燃烧期间保留甲板并持续掉血 · 只有耐久归零后才真正破坏并露出海水',960,C.H-47);
  }
