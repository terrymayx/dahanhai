...
// Projectile core must expose a true visual-height parabola without changing x/y collision logic.
...
P.updateAll(state,.5);
assert(p.z>100,'projectile should be near the apex halfway through the flight');
P.updateAll(state,.5);
assert(p.z>=0&&p.z<1,'projectile should return to sea-level near the intended landing time');

// Real battle shots must actually opt in: player arc higher than enemy arc.
...
assert(playerShot.arcHeight>=140,'player cannonballs must use a clearly visible high arc');
...
assert(enemyShot.arcHeight>0,'enemy cannonballs must also arc');
...
console.log('V8.2 ballistic arc tests passed');
